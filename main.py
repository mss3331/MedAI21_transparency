...
def getModel(model_name, input_channels, number_classes):
    ...
    if model_name.find("Deeplap_resnet") == 0:
...
if __name__=='__main__':
    ...
    root_dir = r"E:\Databases\MedAI_ready_for_training\train"
    colab_dir = "."
    if run_in_colab:
        root_dir = r"/content/MedAI/train"
        colab_dir = "/content/MedAI21"
    ...
    # resize_factor = 2.5
    resize_factor = 2
    target_img_size = (int(531 / resize_factor), int(600 / resize_factor))

    # target_img_size = (int(288/resize_factor),int(384/resize_factor))
    print("resize_factor ",resize_factor)

    ########################### Deeplab versions ###################################
    # [Deeplap_resnet50, Deeplabv3_GRU_ASPP_CombineChannels_resnet50, ]
    ...
    c1 = SegDataset(root_dir, 'C1','images', 'mask',target_img_size,load_to_RAM=load_to_RAM)# SD images
    c2 = SegDataset(root_dir,'C2', 'images', 'mask',target_img_size,load_to_RAM=load_to_RAM)# HD images
    # c3 = SegDataset(root_dir,'C3', 'images', 'mask',target_img_size,load_to_RAM=load_to_RAM)
    #
    # c4 = SegDataset(root_dir, 'C4', 'images', 'mask', target_img_size, load_to_RAM=load_to_RAM)#test 1
    # c5 = SegDataset(root_dir, 'C5', 'images', 'mask', target_img_size, load_to_RAM=load_to_RAM)#test 2
    dummy_dataset = SegDataset(root_dir, 'dummy', 'images', 'mask', target_img_size, load_to_RAM=load_to_RAM)
    megaDataset = ConcatDataset([c2])#c2, c3])#train validate
    ...
    trainDataset, valDataset = trainTestSplit(megaDataset, train_val_ratio)
